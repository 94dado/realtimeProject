...
{
public:
    // a vector with the textures loaded
...
//                cout << "WARNING::ASSIMP:: MODEL WITHOUT UV COORDINATES -> TANGENT AND BITANGENT ARE = 0" << endl;
        ...
        if(mesh->mMaterialIndex >= 0)
        {
            ...
            // 3. Normal maps
            std::vector<TextureStruct> normalMaps = this->loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal");
            textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
            // 4. Height maps
            std::vector<TextureStruct> heightMaps = this->loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height");
